...
/*!***********************************************************************
 @struct	PVRTexLib_PrivateData
 @brief		Structure used for saving texture data to memory.
*************************************************************************/
struct PVRTexLib_PrivateData
{
	void* privateBuffer; ///< Pointer to a user supplied allocation context.
	PVRTuint8* (*pfnPtr)(void* privateBuffer); ///< Callback function to get a pointer to the user allocated memory.
	PVRTuint64 (*pfnSize)(void* privateBuffer); ///< Callback function to get the size of the user allocated memory.
	PVRTuint8* (*pfnRealloc)(void* privateBuffer, PVRTuint64 allocSize); ///< Callback function to reallocate memory on-demand. Return NULL to indicate allocation failure.
};
...
/*!***********************************************************************
 @brief     Creates a new texture header from a pointer that includes a header
			structure as laid out in a file.
 ...
 @return	A new texture header handle OR NULL on failure.
*************************************************************************/
PVR_DLL PVRTexLib_PVRTextureHeader PVRTexLib_CreateTextureHeaderFromData(const void* data);
...
/*!***********************************************************************
 @brief			Save texture data to a memory buffer.
				Caller is responsible for de-allocating memory.
 @param[in] 	header A handle to a previously allocated PVRTexLib_PVRTextureHeader.
 @param[in] 	fileType File container type to wrap the texture data with.
 @param[in] 	data Pointer to texture data to serialise.
 @param[in] 	privateData Pointer to a user supplied allocation context.
 @param[out]	outSize Size, in bytes, of the resulting 'file'/data.
 @return		True if the method succeeds.
				N.B This function may allocate even if it fails.
*************************************************************************/
PVR_DLL bool PVRTexLib_SaveTextureDataToMemory(
	PVRTexLib_CPVRTextureHeader header,
	PVRTexLibFileContainerType fileType,
	const void* data,
	PVRTexLib_PrivateData* privateData,
