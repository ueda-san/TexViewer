...
namespace pvrtexlib
{
	...
	*************************************************************************/
	...
	public:
		...
		*************************************************************************/
		...
		/*!***********************************************************************
		 @brief     Creates a new texture header from a pointer that includes a header structure.
					This functionality is primarily for user-defined file loading.
					Header may be any version of PVR.
		 @param[in]	data Pointer to texture header data
		 @return	A new PVRTextureHeader.
		*************************************************************************/
		inline explicit PVRTextureHeader(const void* data);
		...
		*************************************************************************/
		...
		/*!***********************************************************************
		 @brief		 Saves texture data to a memory buffer in PVR format.
					 The caller is responsible for de-allocating memory.
		 @param[in]	 fileType File container type to wrap the texture data with.
		 @param[in]	 textureData Texture data to save alongside the header.
		...
		*************************************************************************/
		inline bool SaveTextureDataToMemory(PVRTexLibFileContainerType fileType, const void* textureData, PVRTexLib_PrivateData* privateData, PVRTuint64& outSize) const;

		/*!***********************************************************************
		 @brief			Saves the texture data to a memory buffer in PVR format.
		 @param[in]		fileType File container type to wrap the texture data with.
		 @param[in] 	textureData Texture data to save alongside the header.
		 @param[in,out]	outData std::vector containing the resulting data.
		 @return		True if the method succeeds.
		*************************************************************************/
		inline bool SaveTextureDataToMemory(PVRTexLibFileContainerType fileType, const void* textureData, std::vector<PVRTuint8>& outData) const;
		...
		*************************************************************************/
		inline std::unique_ptr<PVRTuint8, void(*)(PVRTuint8*)> SaveTextureDataToMemory(PVRTexLibFileContainerType fileType, const void* textureData, PVRTuint64& outSize) const;
	...
	public:
		...
		*************************************************************************/
		inline bool SaveTextureToMemory(
			...
			PVRTexLib_PrivateData* privateData,
	...
	*************************************************************************/
	...
	PVRTextureHeader::PVRTextureHeader(const void* data)
		: m_hTextureHeader(PVRTexLib_CreateTextureHeaderFromData(data)) {}
	...
	bool PVRTextureHeader::SaveTextureDataToMemory(PVRTexLibFileContainerType fileType, const void* textureData, PVRTexLib_PrivateData* privateData, PVRTuint64& outSize) const
	{
		if (m_hTextureHeader)
		{
			return PVRTexLib_SaveTextureDataToMemory(m_hTextureHeader, fileType, textureData, privateData, &outSize);
		}

		return false;
	}

	bool PVRTextureHeader::SaveTextureDataToMemory(PVRTexLibFileContainerType fileType, const void* textureData, std::vector<PVRTuint8>& outData) const
	{
		if (m_hTextureHeader)
		{
			PVRTuint64 outSize;

			const auto initialSize = PVRTEX3_HEADERSIZE + GetTextureMetaDataSize() + GetTextureDataSize();
			if (initialSize > outData.size())
			{
				outData.resize(static_cast<size_t>(initialSize));
			}
			...
			PVRTexLib_PrivateData privateData = {
				...
				[](void* privateData) { 
					auto buffer = static_cast<const std::vector<PVRTuint8>*>(privateData);
					return static_cast<PVRTuint64>(buffer->size());
				},
			...
			if (PVRTexLib_SaveTextureDataToMemory(m_hTextureHeader, fileType, textureData, &privateData, &outSize)) {
	...
	std::unique_ptr<PVRTuint8, void(*)(PVRTuint8*)> PVRTextureHeader::SaveTextureDataToMemory(PVRTexLibFileContainerType fileType, const void* textureData, PVRTuint64& outSize) const
	{
		struct PrivateDataBuffer
		{
			std::unique_ptr<PVRTuint8, void(*)(PVRTuint8*)> memory;
			PVRTuint64 size;
		} privateBuffer { {nullptr, [](PVRTuint8* data) { free(data); }}, 0};

		if (m_hTextureHeader)
		{
			privateBuffer.size = PVRTEX3_HEADERSIZE + GetTextureMetaDataSize() + GetTextureDataSize();
			privateBuffer.memory.reset(static_cast<PVRTuint8*>(std::realloc(privateBuffer.memory.get(), static_cast<size_t>(privateBuffer.size))));

			PVRTexLib_PrivateData privateData = {
				static_cast<void*>(&privateBuffer),
				[](void* privateBuffer) {
					auto instance = static_cast<PrivateDataBuffer*>(privateBuffer);
					return instance->memory.get();
				},
				[](void* privateBuffer) {
					auto instance = static_cast<PrivateDataBuffer*>(privateBuffer);
					return instance->size;
				},
				[](void* privateBuffer, PVRTuint64 allocSize) {
					auto instance = static_cast<PrivateDataBuffer*>(privateBuffer);
					auto currentPtr = instance->memory.release();
					assert(allocSize <= std::numeric_limits<size_t>::max());
					auto newPtr = std::realloc(currentPtr, static_cast<size_t>(allocSize));

					if (newPtr)
					{
						instance->memory.reset(static_cast<PVRTuint8*>(newPtr));
						instance->size = allocSize;
						return instance->memory.get();
					}
					else
					{
						instance->memory.reset(static_cast<PVRTuint8*>(currentPtr));
						return static_cast<PVRTuint8*>(nullptr);
					}
				}
			};
			...
			if (!PVRTexLib_SaveTextureDataToMemory(m_hTextureHeader, fileType, textureData, &privateData, &outSize))
			{
				privateBuffer.memory.reset();
			}
		}

		return std::move(privateBuffer.memory);
	}
	...
	*************************************************************************/
		...
		PVRTuint64& outSize) const
	{
		if (m_hTexture)
		{
			return PVRTexLib_SaveTextureToMemory(m_hTexture, fileType, privateData, &outSize);
		}
		...
		std::vector<PVRTuint8>& outData) const
	{
		if (m_hTexture)
		{
			...
			if (PVRTexLib_SaveTextureToMemory(m_hTexture, fileType, &privateData, &outSize))
			{
				outData.resize(static_cast<size_t>(outSize));
				return true;
			}
