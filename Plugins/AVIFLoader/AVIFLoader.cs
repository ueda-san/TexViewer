...
namespace AVIFLoader
{
    public class AVIFLoader : ITexViewerPlugin
    {
        public string PluginName => "AVIF Loader";
        public string PluginDescription => "Load images using libavif";
        public string SupportedExtensions => "avif";
        public UInt16 DefaultPriority => 310;

        [DllImport("AVIFLoaderLib.dll", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
        public static extern IntPtr get_version();
        [DllImport("AVIFLoaderLib.dll", CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
        public static extern int load_file(string file, out LoadResult result, uint opt);
        [DllImport("AVIFLoaderLib.dll", CallingConvention = CallingConvention.Cdecl)]
